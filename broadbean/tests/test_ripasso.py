...
from broadbean.ripasso import applyRCFilter, applyInverseRCFilter
import numpy as np
import pytest
...
def squarewave():
    ...
    for n in range(periods):
        array[int(n * NUM_POINTS / periods): int((2 * n + 1)* NUM_POINTS / 2 / periods)] = 1
...
def test_rc_filter(squarewave):
    ...
    for filter_type in ["HP", "LP"]:
        ...
        signal1_filtered2 = applyInverseRCFilter(signal1_filtered, SR, filter_type, f_cut=12, order=1)
...
def test_output_seqx_file(squarewave):
    ...
    elem1.addArray(1, signal1_filtered, SR, m1=np.zeros(NUM_POINTS), m2=np.zeros(NUM_POINTS))
    seq1 = bb.Sequence()
