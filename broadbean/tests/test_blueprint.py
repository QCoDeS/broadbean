...
def test_tophat_changeargument(blueprint_tophat, name, arg, newval, argslist):
    blueprint_tophat.change_blueprint_argument(name, arg, newval)
...
def test_tophat_changeargument_replaceeverywhere(
    blueprint_tophat, name, arg, newval, argslist
):
    blueprint_tophat.change_blueprint_argument(
        name, arg, newval, replaceeverywhere=True
    )
    assert blueprint_tophat._argslist == argslist
...
def test_tophat_changeargument_valueerror(blueprint_tophat, name, arg):
    with pytest.raises(ValueError):
        blueprint_tophat.change_blueprint_argument(name, arg, 0)
...
def test_tophat_insert_funlist(blueprint_tophat, pos, func, funlist):
    blueprint_tophat.insert_segment(pos, func, args=(1, 0), dur=1)
...
def test_tophat_insert_argslist(blueprint_tophat, pos, argslist):
    blueprint_tophat.insert_segment(pos, ramp, newargs, dur=1)
    assert blueprint_tophat._argslist == argslist
...
def test_tophat_insert_namelist(blueprint_tophat, pos, name, namelist):
    blueprint_tophat.insert_segment(pos, ramp, newargs, name=name, dur=0.5)
...
def test_tophat_remove_namelist(blueprint_tophat, name):
    if name in blueprint_tophat._namelist:
        blueprint_tophat.remove_segment(name)
...
def test_tophat_remove_segmentmarker(blueprint_tophat):
    ...
    with pytest.raises(ValueError):
        blueprint_tophat.remove_segment_marker("ramp", 3)
    with pytest.raises(ValueError):
        blueprint_tophat.remove_segment_marker("no such name", 3)

    # Adding and removing should be equivalent to NOOP
    bpc = blueprint_tophat.copy()
    blueprint_tophat.set_segment_marker("ramp", (0, 0.1), 1)
    blueprint_tophat.remove_segment_marker("ramp", 1)
    assert bpc == blueprint_tophat

    with pytest.raises(KeyError):
        bpc.remove_segment_marker("no such name", 1)
